...
for i, k in enumerate(range(9, -1, -1)):
    ...
    for order in orders:
        ...
        model_name = "{}_data{}_bs{}_epoch{}_adam_lr{}_lrdecay{}".format(net.__class__.__name__, data, batch_size,
                                                                         n_epochs, lr, lr_decay)
        ...
        if resume_net:
            ...
            # change the state for shuffling orders
            original_state_dict = copy.deepcopy(state_dict)
            for idx, layer in enumerate(order):
                state_dict['conv{}.weight'.format(idx + 1)] = original_state_dict['conv{}.weight'.format(layer)]
                state_dict['conv{}.bias'.format(idx + 1)] = original_state_dict['conv{}.bias'.format(layer)]
                if 'ln' in model_name:
