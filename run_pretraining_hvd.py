...
import os
import modeling_new_layernorm as modeling
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
                     ...
                     use_one_hot_embeddings, add_weight, weight_type, weight_act, linear_attention):
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            attention_weight_summary = [
                tf.summary.scalar("attention_weight_layer_{}".format(i), all_layer_attention_weight[i])
...
def main(_):
    ...
    # input_files = tf.io.gfile.glob(FLAGS.input_file + "*.tfrecord")

    # input_files = []
    # for input_pattern in FLAGS.input_file.split(","):
    #     files = tf.gfile.Glob(input_pattern)
    #     for file in files:
    #         if int(file.split('/')[-1].split('.')[0][5:]) < 100:
