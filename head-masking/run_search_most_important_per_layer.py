"""For each layer, exhaustively search for the most important head"""
...
def main(_):
    ...
    tf.compat.v1.logging.info("Running exhaustive search for the most important head for each layer!")

    # we want to mask out the individual head and then evaluate. So there are 12 layers * 12 heads results.
    n_layers = 12
    n_heads = 12
    folder = FLAGS.output_dir
    save_file = 'search_most_important_per_layer.pickle'
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        for l in range(n_layers):
            ...
            for h in range(n_heads):
                ...
                head_mask = [head for head in range(n_heads) if head != h]
                layer_mask = [l] * (n_heads - 1)
                ...
                for name, val in zip(metric_name, eval_metric_val):
                    if name == 'accuracy':
                        print("Mask out heads in (Layer {}, Head combination {}) | {}: {}"
                              .format(l, head_mask, name, val))
                        if val > best_one[0]:
                            best_one = [val, h]
