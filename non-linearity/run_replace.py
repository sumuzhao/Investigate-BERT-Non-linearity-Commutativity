"""Replace the certain parts by trained approximators"""
...
flags.DEFINE_string(
    "approximator_checkpoint", None,
    "Approximator checkpoint.")
...
flags.DEFINE_bool("random_init", False, "In fine_then_replace, random init the non-linearity")
...
def main(_):
    ...
    tf.compat.v1.logging.info("Running replacement experiments! ")
    ...
    if FLAGS.approximator_setting == 'HS_Self_Attention_FF':
        model = modeling_replace_sa_ff.BertModel(
            config=bert_config,
    ...
    for layer in layers:
        approximator_checkpoint.append(tf.train.latest_checkpoint(FLAGS.approximator_checkpoint +
                                                                  "layer_{}/".format(layer)))
    tvars = tf.compat.v1.trainable_variables()
    var_init = [v for v in tvars if 'non-linearity' not in v.name
                and 'output_weights' not in v.name
                and 'output_bias' not in v.name]
    var_output = [v for v in tvars if 'output_weights' in v.name or "output_bias" in v.name]
    var_approximator = []
    for layer in layers:
        var_approximator.append([v for v in tvars if "bert/encoder/layer_{}/non-linearity".format(layer) in v.name])
    ...
    for i in range(len(layers)):
        saver_approximator.append(tf.train.Saver(var_approximator[i]))

    var_all = var_init + [v for v_a in var_approximator for v in v_a] + var_output
    train_op = optimization.create_optimizer(loss=loss,
                                             ...
                                             use_tpu=False,
                                             tvars=var_all)
    saver_all = tf.train.Saver(var_list=var_all, max_to_keep=1)
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        if not FLAGS.random_init:
            tf.logging.info("Initialize the non-linearity from checkpoint")
            for i in range(len(layers)):
                saver_approximator[i].restore(sess, approximator_checkpoint[i])
        else:
            tf.logging.info("Randomly initialize the non-linearity")
            ...
            for n in range(epochs):
                ...
                for b in range(num_batch):
                    ...
                    if step % 100 == 0 or step % num_batch == 0 or step == 1:
                        ...
                        print("Replacing {} | Epoch: %4d/%4d | Batch: %4d/%4d | {} | {}"
                              .format(layer_folder_name, train_str, eval_str) % (n, epochs, b, num_batch))

                    if step % num_batch == 0:
                        saver_all.save(sess, log_dir + 'replace_{}'.format(layer_folder_name), global_step=step)
        ...
        with tf.io.gfile.GFile(FLAGS.output_dir + 'results.txt', 'a') as writer:
            ...
            writer.write("Replacing {}: Dev start: {} | Dev end: {}　| Test end: {}\n"
