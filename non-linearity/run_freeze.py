"""Freezing experiments, freeze some parts of BERT. """
...
def main(_):
    ...
    if FLAGS.freeze_part == 'ffn':
        var_freeze = [v for i in freeze_layers for v in var_init if "layer_{}/intermediate/dense".format(i) in v.name or
                      "layer_{}/output/dense".format(i) in v.name]
    elif FLAGS.freeze_part == 'att':
        var_freeze = [v for i in freeze_layers for v in var_init if "layer_{}/attention/output/dense".format(i) in v.name or
                      "layer_{}/attention/self".format(i) in v.name]
    elif FLAGS.freeze_part == 'encoder':
        var_freeze = [v for i in freeze_layers for v in var_init if "layer_{}/".format(i) in v.name]
    elif FLAGS.freeze_part == 'pooler+embedding':
        var_freeze = [v for v in var_init if "pooler" in v.name or "embeddings" in v.name]
        ...
        var_freeze = [v for i in freeze_layers for v in var_init if "layer_{}/".format(i) not in v.name]
    elif FLAGS.freeze_part == 'allbutonelayer+pooler+output':
        var_freeze = [v for i in freeze_layers for v in var_init if "layer_{}/".format(i) not in v.name and 'pooler' not in v.name]
    ...
    with tf.compat.v1.Session(config=config) as sess:
            ...
            for n in range(epochs):
                ...
                for b in range(num_batch):
                    ...
                    if step % 100 == 0 or step % num_batch == 0 or step == 1:
                        ...
                        print("Freezing {} | Epoch: %4d/%4d | Batch: %4d/%4d | {} | {}"
                              .format(layer_folder_name, train_str, eval_str) % (n, epochs, b, num_batch))

                    if step % num_batch == 0:
                        saver_all.save(sess, log_dir + 'freeze_{}'.format(layer_folder_name), global_step=step)
        ...
        with tf.io.gfile.GFile(FLAGS.output_dir + 'results.txt', 'a') as writer:
            ...
            writer.write("Freezing {}: Dev start: {} | Dev end: {}　| Test end: {}\n"
