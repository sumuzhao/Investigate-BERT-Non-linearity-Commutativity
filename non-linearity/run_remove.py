"""Remving experiments, remove some parts of BERT. """
...
flags.DEFINE_string("remove_part", None, "What part to be removed.")

flags.DEFINE_string("freeze_part", None, "What part to be frozen.")

flags.DEFINE_integer("tf_seed", None, "TF seed for the random runs. ")
...
def main(_):
    ...
    tf.compat.v1.logging.info("Running removing experiments!")

    num_train_steps = num_actual_train_examples // batch_size * epochs
    num_warmup_steps = int(num_train_steps * FLAGS.warmup_proportion)

    # get the layer(s) which need removing.
    if FLAGS.layers is None:
        raise ValueError("In removing experiments, layers must not be None. ")
    layer_folder_name = FLAGS.layers
    layers = list(map(int, FLAGS.layers.split(',')))
    layers.sort()
    print("Current removed layers: ", layers)
    ...
    var_remove = []
    if FLAGS.remove_part == 'MLP':
        # All vars in MLP
        var_remove = [v for i in layers for v in var_init if "layer_{}/intermediate/dense".format(i) in v.name or
                      "layer_{}/output/dense".format(i) in v.name]
    elif FLAGS.remove_part == 'FF_MLP':
        # Only vars from linear layer in MLP
        var_remove = [v for i in layers for v in var_init if "layer_{}/output/dense".format(i) in v.name]
    elif FLAGS.remove_part == 'FFGelu_MLP':
        # Only vars from non-linear layer in MLP
        var_remove = [v for i in layers for v in var_init if "layer_{}/intermediate/dense".format(i) in v.name]
    elif FLAGS.remove_part == 'attention':
        # All vars in attention
        var_remove = [v for i in layers for v in var_init if "layer_{}/attention".format(i) in v.name]
    elif FLAGS.remove_part == 'self_attention_ff':
        # Vars in self-attention block and the following linear layer
        var_remove = [v for i in layers for v in var_init if "layer_{}/attention/self".format(i) in v.name or
                      "layer_{}/attention/output/dense".format(i) in v.name]
    elif FLAGS.remove_part == 'encoder':
        # All vars in encoder, including layer norm, beta -> 0, gamma -> 1
        var_remove = [v for i in layers for v in var_init if "layer_{}/".format(i) in v.name]
    elif FLAGS.remove_part == 'encoder_except_layernorm':
        # All vars in encoder except layer norm (beta, gamma)
        var_remove = [v for i in layers for v in var_init if "layer_{}/".format(i) in v.name and "LayerNorm" not in v.name]

    # set 0 to vars to be removed
    print("Vars to be removed (assigned as zero and not trained)")
    for var in var_remove:
        print(var)
        # take care of the gamma value of layer norm
        if "LayerNorm/gamma" in var.name:
    ...
    elif FLAGS.freeze_part == 'allbut1-3,12':
        finetune_part = list(range(3)) + [11]
    elif FLAGS.freeze_part == 'allbut5-7,12':
        finetune_part = list(range(4, 7)) + [11]
    ...
    if not FLAGS.load_from_finetuned:
        # Init from Model0
        saver_init = tf.train.Saver([v for v in var_init if v not in var_remove])
    else:
        # Init from Model1
        saver_init = tf.train.Saver([v for v in var_init + var_output if v not in var_remove])

    var_train = [v for v in var_init + var_output if v not in var_remove and v not in var_freeze]
    ...
    with tf.compat.v1.Session(config=config) as sess:
            ...
            for n in range(epochs):
                ...
                for b in range(num_batch):
                    ...
                    if step % 100 == 0 or step % num_batch == 0 or step == 1:
                        ...
                        print("Removing {} | Epoch: %4d/%4d | Batch: %4d/%4d | {} | {}"
                              .format(layer_folder_name, train_str, eval_str) % (n, epochs, b, num_batch))

                    if step % num_batch == 0:
                        saver_all.save(sess, log_dir + 'remove_{}'.format(layer_folder_name), global_step=step)
        ...
        with tf.io.gfile.GFile(FLAGS.output_dir + 'results.txt', 'a') as writer:
            ...
            writer.write("Removing {}: Dev start: {} | Dev end: {}　| Test end: {}\n"
