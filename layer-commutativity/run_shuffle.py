...
flags.DEFINE_string("shuffle_setting", None, "How to shuffle layers")

flags.DEFINE_string("layers", None, "only digits if single layer, splited by ',' if multiple layers, e.g. 1,2,3")

flags.DEFINE_integer("np_seed", None, "Numpy seed")

flags.DEFINE_string("freeze_part", None, "What part to be frozen.")
...
def main(_):
    ...
    tf.compat.v1.logging.info("Running shuffling layers!")
    ...
    # shuffle layers
    shuffle_layer_order = list(range(12))
    if FLAGS.shuffle_setting == '1-12':
        print("Randomly shuffle all layers! ")
        np.random.shuffle(shuffle_layer_order)
    elif FLAGS.shuffle_setting == 'fix1-4':
        print("Fix layer 1-4 then randomly shuffle [5-12] together! ")
        temp = list(range(4, 12))
        np.random.shuffle(temp)
        shuffle_layer_order = [0, 1, 2, 3] + temp
    elif FLAGS.shuffle_setting == 'fix1-4_5-8,9-12':
        print("Fix layer 1-4 then randomly shuffle [5-8], [9-12] within groups! ")
        temp1 = list(range(4, 8))
        temp2 = list(range(8, 12))
        np.random.shuffle(temp1)
        np.random.shuffle(temp2)
        shuffle_layer_order = [0, 1, 2, 3] + temp1 + temp2
    elif FLAGS.shuffle_setting == 'fix5-8':
        print("Fix layer 5-8 then randomly shuffle [1-4,9-12] together! ")
        temp = list(range(0, 4)) + list(range(8, 12))
        np.random.shuffle(temp)
        shuffle_layer_order = temp[:4] + [4, 5, 6, 7] + temp[4:]
    elif FLAGS.shuffle_setting == 'fix5-8_1-4,9-12':
        print("Fix layer 5-8 then randomly shuffle [1-4], [9-12] within groups! ")
        temp1 = list(range(0, 4))
        temp2 = list(range(8, 12))
        np.random.shuffle(temp1)
        np.random.shuffle(temp2)
        shuffle_layer_order = temp1 + [4, 5, 6, 7] + temp2
    elif FLAGS.shuffle_setting == 'fix9-12':
        print("Fix layer 9-12 then randomly shuffle [1-8]! ")
        temp = list(range(0, 8))
        np.random.shuffle(temp)
        shuffle_layer_order = temp + [8, 9, 10, 11]
    elif FLAGS.shuffle_setting == 'fix9-12_1-4,5-8':
        print("Fix layer 9-12 then randomly shuffle [1-4], [5-8] within groups! ")
        temp1 = list(range(0, 4))
        temp2 = list(range(4, 8))
        np.random.shuffle(temp1)
        np.random.shuffle(temp2)
        shuffle_layer_order = temp1 + temp2 + [8, 9, 10, 11]
    elif FLAGS.shuffle_setting == '1-4,5-12':
        print("Shuffle within layer [1-4] and [5-12]! ")
        temp1 = list(range(0, 4))
        temp2 = list(range(4, 12))
        np.random.shuffle(temp1)
        np.random.shuffle(temp2)
        shuffle_layer_order = temp1 + temp2
    elif FLAGS.shuffle_setting == '1-4,5-8,9-12':
        print("Shuffle within layer [1-4], [5-8] and [9-12]! ")
        temp1 = list(range(0, 4))
        temp2 = list(range(4, 8))
        temp3 = list(range(8, 12))
        np.random.shuffle(temp1)
        np.random.shuffle(temp2)
        np.random.shuffle(temp3)
        shuffle_layer_order = temp1 + temp2 + temp3
    elif FLAGS.shuffle_setting == 'albert-like':
        if FLAGS.layers is None:
            raise ValueError("When in shuffling experiments, either shuffle_setting or layers cannot be none. ")
        else:
            n_layer = 12 if embed_dim == 768 else 6
            shuffle_layer_order = list(map(int, FLAGS.layers.split(','))) * n_layer
    elif FLAGS.shuffle_setting is None:
        if FLAGS.layers is None:
            raise ValueError("When in shuffling experiments, either shuffle_setting or layers cannot be none. ")
        else:
            shuffle_layer_order = list(map(int, FLAGS.layers.split(',')))
    layer_folder_name = ','.join(map(str, shuffle_layer_order))
    print("Current layer order: ", shuffle_layer_order)
    ...
    model = modeling.BertModel(
        ...
        layer_order=shuffle_layer_order,
        use_estimator=False)
    ...
    # Freeze variables
    finetune_part = []
    # in terms of layer name
    if FLAGS.freeze_part == 'allbut1-4':
        finetune_part = range(4)
    elif FLAGS.freeze_part == 'allbut5-8':
        finetune_part = range(4, 8)
    elif FLAGS.freeze_part == 'allbut9-12':
        finetune_part = range(8, 12)
    elif FLAGS.freeze_part == 'allbut5-12':
        finetune_part = range(4, 12)
    elif FLAGS.freeze_part == 'allbut1-8':
        finetune_part = range(8)
    elif FLAGS.freeze_part == 'allbut1-4,9-12':
        finetune_part = list(range(4)) + list(range(8, 12))
    elif FLAGS.freeze_part == 'allbut1-12':
        finetune_part = range(12)
    # in terms of layer index (position)
    elif FLAGS.freeze_part == 'allbut1-4_idx':
        finetune_part = [shuffle_layer_order[l] for l in range(4)]
    elif FLAGS.freeze_part == 'allbut5-8_idx':
        finetune_part = [shuffle_layer_order[l] for l in range(4, 8)]
    elif FLAGS.freeze_part == 'allbut9-12_idx':
        finetune_part = [shuffle_layer_order[l] for l in range(8, 12)]
    elif FLAGS.freeze_part == 'allbut1-8_idx':
        finetune_part = [shuffle_layer_order[l] for l in range(8)]
    elif FLAGS.freeze_part == 'allbut5-12_idx':
        finetune_part = [shuffle_layer_order[l] for l in range(4, 12)]
    elif FLAGS.freeze_part == 'allbut1-4,9-12_idx':
        finetune_part = [shuffle_layer_order[l] for l in list(range(4)) + list(range(8, 12))]
    elif FLAGS.freeze_part == 'allbut1-12_idx':
        finetune_part = [shuffle_layer_order[l] for l in range(12)]
    if FLAGS.freeze_part == 'nothing':
        var_freeze = []
    elif FLAGS.freeze_part == 'allbutoutput_layer':
        var_freeze = var_init
    else:
        var_freeze = [v for v in var_init if all("layer_{}/".format(layer) not in v.name for layer in finetune_part)]

    print("Vars to be frozen")
    for var in var_freeze:
        print(var)
    ...
    var_train = [v for v in var_init + var_output if v not in var_freeze]
    ...
    with tf.compat.v1.Session(config=config) as sess:
            ...
            for n in range(epochs):
                ...
                for b in range(num_batch):
                    ...
                    if step % 100 == 0 or step % num_batch == 0 or step == 1:
                        ...
                        print("Shuffle layer order {} | Epoch: %4d/%4d | Batch: %4d/%4d | {} | {}"
                              .format(layer_folder_name, train_str, eval_str) % (n, epochs, b, num_batch))

                    if step % num_batch == 0:
                        saver_all.save(sess, log_dir + 'shuffle_{}'.format(layer_folder_name), global_step=step)
        ...
        if FLAGS.do_eval:
            ...
            if task_name != 'stsb':
                ...
                writer_output.write("ID \t Label \t Ground Truth\n")
            for (i, pred) in enumerate(preds):
                if task_name != 'stsb':
                    ...
                    writer_output.write("{} \t {} \t {} \n".format(i, pred, eval_label_ids_lst[i]))
            writer_output.close()
        ...
        with tf.io.gfile.GFile(FLAGS.output_dir + 'results.txt', 'a') as writer:
            ...
            writer.write("Shuffle layer order {}: Dev start: {} | Dev end: {}　| Test end: {}\n"
                         .format(layer_folder_name,
                                 ','.join(eval_start),
                                 ','.join(eval_end),
                                 ','.join(test_end)))
        tf.logging.info("***** Writing finishes *****")
