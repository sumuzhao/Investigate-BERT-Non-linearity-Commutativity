...
def main(_):
    ...
    if len(layers) == 2:
        layers.sort()
        if layer1 != layer2:
            layer_order[layer1], layer_order[layer2] = layer_order[layer2], layer_order[layer1]
        else:
            raise ValueError("Two layers should be different! ")
    else:
        layer_order = layer_order[:layers[0]] + layers[::-1] + layer_order[layers[-1] + 1:]
    ...
    with tf.compat.v1.Session(config=config) as sess:
            ...
            for n in range(epochs):
                ...
                for b in range(num_batch):
                    ...
                    if step % 100 == 0 or step % num_batch == 0 or step == 1:
                        ...
                        print("Swap layer order {} | Epoch: %4d/%4d | Batch: %4d/%4d | {} | {}"
                              .format(layer_folder_name, train_str, eval_str) % (n, epochs, b, num_batch))

                    if step % num_batch == 0:
                        saver_all.save(sess, log_dir + 'swap_{}'.format(layer_folder_name), global_step=step)
        ...
        with tf.io.gfile.GFile(FLAGS.output_dir + 'results.txt', 'a') as writer:
            ...
            writer.write("Swap layer order {}: Dev start: {} | Dev end: {}　| Test end: {}\n"
